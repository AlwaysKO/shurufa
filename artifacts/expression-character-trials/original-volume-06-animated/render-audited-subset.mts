...
import {ORIGINAL_VOLUME06_ITEMS,renderReferenceCharacterGif,resolveReferenceMaster} from '../../../server/src/expression/referenceCharacterRenderer.js';
...
for(const item of ORIGINAL_VOLUME06_ITEMS){
 try {rendered.push(await renderReferenceCharacterGif(await readFile(await resolveReferenceMaster(root,item.masterFile)),item));}
  ...
  if(item.id!=='pig-content'||!(error instanceof Error)||!error.message.startsWith('GIF审计失败：'))throw error;
  const issues=JSON.parse(error.message.slice('GIF审计失败：'.length));
  if(!Array.isArray(issues)||issues.length!==1||issues[0].id!=='pig-content'||issues[0].field!=='loopClosure')throw error;
  failed.push({id:item.id,caption:item.caption,error:String(error)});
 }
}
if(rendered.length!==11||failed.length!==1||failed[0].id!=='pig-content')throw new Error('审计结果变化，请重新审核分流，不自动扩大输出');
