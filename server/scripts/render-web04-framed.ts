...
import { renderReferenceCharacterGif, ORIGINAL_WEB04_ITEMS } from '../src/expression/referenceCharacterRenderer.js';
import { publishDirectoryAtomically } from '../src/expression/prototypePublication.js';

if(process.argv.length>2)throw new Error('仅处理web04固定新稿，不接受参数');
const root=fileURLToPath(new URL('../../artifacts/expression-character-trials/web-original-04-animated/',import.meta.url));
...
for(const spec of specs){
 const item=ORIGINAL_WEB04_ITEMS.find(x=>x.id===spec.id);
 if(!item)throw new Error('非web04固定素材');
