...
import { renderSceneRich06Gif, validateSceneRich06Manifest } from '../src/expression/sceneRich06Renderer.js';

// 仅接受明确登记的评审批次，不接受外部输出路径，不能改写正式素材目录。
async function main() {
  if (process.argv.length > 2) throw new Error('本脚本不接受额外参数');
  const batchId = 'scene-rich-06';
  ...
  const manifest = validateSceneRich06Manifest(JSON.parse(await readFile(join(source,'manifest.json'),'utf8')));
  ...
  for (const item of manifest.items) {
    ...
    rendered.push({item,...await renderSceneRich06Gif(await readFile(path),item)});
  }
