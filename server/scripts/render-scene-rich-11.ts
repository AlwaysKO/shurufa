...
import { renderSceneRich11Gif, validateSceneRich11Manifest, resolveSceneRich11Master } from '../src/expression/sceneRich11Renderer.js';

// 仅接受明确登记的评审批次，不接受外部输出路径，不能改写正式素材目录。
async function main() {
  if (process.argv.length > 2) throw new Error('本脚本不接受额外参数');
  const batchId = 'scene-rich-11';
  ...
  const manifest = validateSceneRich11Manifest(JSON.parse(await readFile(join(source,'manifest.json'),'utf8')));
  if (manifest.batchId !== batchId) throw new Error('清单批次与所选批次不一致');
  const rendered = [];
  for (const item of manifest.items) {
    const path = await resolveSceneRich11Master(source,item.masterFile);
    rendered.push({item,...await renderSceneRich11Gif(await readFile(path),item)});
  }
