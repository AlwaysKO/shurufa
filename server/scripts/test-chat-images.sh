...
CLUSTER="$(mktemp -d /tmp/shurufa-chat-images.XXXXXXXX)"
printf chat-images-only > "$CLUSTER/test-instance-only"
...
"$PG_BIN/createdb" -h "$CLUSTER/socket" -p 5432 chat_images_test
cd "$ROOT"
CHAT_IMAGES_TEST_CLUSTER="$CLUSTER" ./server/node_modules/.bin/vitest run server/src/api/chatImages.postgres.test.ts "$@"
