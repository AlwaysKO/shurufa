...
async function main() {
  ...
  await publishDirectoryAtomically(join(root,'output'),async output=>{
    ...
    const report={status:'trial-only',sourceType:original?'ai-original':'user-provided-reference',licenseStatus:original?'pending-review':'unverified',publicationAllowed:false,
      staticCharacterReview:(volume10 || volume09 || volume08 || volume07 || volume06 || volume05 || volume04 || volume03 || volume02 || volume || hurt || great || speechless || sleepy || received || look)?'pending':'approved',humanAnimationReview:'pending',total:rendered.length,pass:rendered.length,fail:0,
    ...
    await writeFile(join(output,'preview.html'),`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${(volume10 || volume09 || volume08 || volume07 || volume06 || volume05 || volume04 || volume03 || volume02 || volume || hurt || great || speechless || sleepy || received || look)?'形象与动态待审·原创试稿':'已确认形象·动态试稿'}</title><style>body{font-family:system-ui;background:#eee;padding:24px}main{display:flex;gap:24px;flex-wrap:wrap}article{background:white;padding:16px}img{width:240px;height:240px}</style><h1>${original?'原创角色':'熊猫头 / 蘑菇头'}动态试稿</h1><p>每张4秒、20帧，12格姿势顺序播放。${(volume10 || volume09 || volume08 || volume07 || volume06 || volume05 || volume04 || volume03 || volume02 || volume || hurt || great || speechless || sleepy || received || look)?'形象与动态均待审':'形象已确认，动态效果待审'}；未加入APK或接口。</p><main>${rendered.map(r=>`<article><h2>${r.item.caption}</h2><img src="gifs/${r.item.id}.gif" alt="${r.item.caption}"><p>${(r.gif.length/1024).toFixed(1)} KiB</p><a href="gifs/${r.item.id}.gif">原始GIF</a></article>`).join('')}</main></html>`);
  });
