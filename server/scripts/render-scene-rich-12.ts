...
import { renderSceneRich12Gif, validateSceneRich12Manifest, resolveSceneRich12Master } from '../src/expression/sceneRich12Renderer.js';

// 仅接受明确登记的评审批次，不接受外部输出路径，不能改写正式素材目录。
async function main() {
  if (process.argv.length > 2) throw new Error('本脚本不接受额外参数');
  const batchId = 'scene-rich-12';
  ...
  const manifest = validateSceneRich12Manifest(JSON.parse(await readFile(join(source,'manifest.json'),'utf8')));
  if (manifest.batchId !== batchId) throw new Error('清单批次与所选批次不一致');
  const rendered = [];
  for (const item of manifest.items) {
    const path = await resolveSceneRich12Master(source,item.masterFile);
    rendered.push({item,...await renderSceneRich12Gif(await readFile(path),item)});
  }
