...
import { parseSceneRichBatchArgs, renderSceneRichGif, validateSceneRichManifest } from '../src/expression/sceneRichRenderer.js';

// 仅接受明确登记的评审批次，不接受外部输出路径，不能改写正式素材目录。
async function main() {
  const batchId = parseSceneRichBatchArgs(process.argv.slice(2));
  ...
  const manifest = validateSceneRichManifest(JSON.parse(await readFile(join(source,'manifest.json'),'utf8')));
  if (manifest.batchId !== batchId) throw new Error('清单批次与所选批次不一致');
  const rendered = [];
  for (const item of manifest.items) {
    const path = await realpath(join(source,item.masterFile));
    if (!path.startsWith(source + sep)) throw new Error('母版符号链接越界');
    rendered.push({item,...await renderSceneRichGif(await readFile(path),item)});
  }
  ...
  const report = {batchId:manifest.batchId,status:'review-only',total:4,pass:4,fail:0,expectedTotal:4,humanReview:'pending',items:rendered.map(r=>({...r.item,...r.audit}))};
