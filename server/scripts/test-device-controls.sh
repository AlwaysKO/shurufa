...
CLUSTER="$(mktemp -d /tmp/shurufa-device-controls.XXXXXXXX)"
printf device-controls-only > "$CLUSTER/test-instance-only"
...
"$PG_BIN/createdb" -h "$CLUSTER/socket" -p 5432 device_controls_test
cd "$ROOT"
DEVICE_CONTROLS_TEST_CLUSTER="$CLUSTER" ./server/node_modules/.bin/vitest run server/src/api/deviceControls.postgres.test.ts "$@"
