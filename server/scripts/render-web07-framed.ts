...
import { renderReferenceCharacterGif, ORIGINAL_WEB07_ITEMS } from '../src/expression/referenceCharacterRenderer.js';
import { publishDirectoryAtomically } from '../src/expression/prototypePublication.js';

if(process.argv.length>2)throw new Error('仅处理web07固定新稿，不接受参数');
const root=fileURLToPath(new URL('../../artifacts/expression-character-trials/web-original-07-animated/',import.meta.url));
...
for(const spec of specs){
 const item=ORIGINAL_WEB07_ITEMS.find(x=>x.id===spec.id);
 if(!item)throw new Error('非web07固定素材');
