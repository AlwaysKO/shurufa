...
CLUSTER="$(mktemp -d /tmp/shurufa-chat-continuity.XXXXXXXX)"
printf chat-continuity-only > "$CLUSTER/test-instance-only"
...
"$PG_BIN/createdb" -h "$CLUSTER/socket" -p 5432 chat_continuity_test
cd "$ROOT"
CHAT_CONTINUITY_TEST_CLUSTER="$CLUSTER" ./server/node_modules/.bin/vitest run server/src/api/chatContinuity.postgres.test.ts "$@"
