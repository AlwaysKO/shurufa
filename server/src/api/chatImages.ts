...
export function createChatImagesRouter(pool: pg.Pool): Router {
  ...
  router.get('/images/adjacent', async (req, res, next) => {
    ...
    if (!positiveId(conversationId) || !positiveId(assetId) || !validUuid(messageId)
    ...
    try {
      const result = await pool.query(`WITH images AS (
        ...
        WHERE m.user_id=$1 AND m.conversation_id=$2 AND a.mime_type LIKE 'image/%'
        GROUP BY m.id, a.id
      ), ordered AS (
      ...
      [res.locals.userId, conversationId, messageId, assetId, direction === 'next' ? 1 : -1]);
  ...
  router.post('/images/delete-batch', async (req, res, next) => {
    ...
    if (body?.confirm !== 'DELETE' || !validUuid(userId) || !positiveId(body?.conversation_id)
    ...
    try {
      ...
      try {
        ...
        const conversation = await db.query('SELECT id FROM chat_conversation WHERE id=$1 AND user_id=$2 FOR UPDATE', [body.conversation_id, userId]);
        ...
        await db.query(`SELECT id FROM chat_message WHERE conversation_id=$1 AND user_id=$2 AND id=ANY($3::uuid[]) ORDER BY id FOR UPDATE`, [body.conversation_id, userId, messageIds]);
        const links = await db.query<{ message_id: string; asset_id: string }>(`SELECT ma.message_id, ma.asset_id
          ...
          WHERE m.conversation_id=$1 AND m.user_id=$2 AND a.user_id=$2 AND a.mime_type LIKE 'image/%'
          ORDER BY ma.asset_id, ma.message_id, ma.role FOR UPDATE OF ma, a`, [body.conversation_id, userId, JSON.stringify(images)]);
        ...
        const messages = await db.query(`DELETE FROM chat_message m WHERE m.user_id=$1 AND m.conversation_id=$2 AND m.id=ANY($3::uuid[])
          AND m.message_type='image' AND COALESCE(m.text,'') IN ('','图片','截图')
          AND NOT EXISTS (SELECT 1 FROM chat_message_asset ma WHERE ma.message_id=m.id)`, [userId, body.conversation_id, messageIds]);
