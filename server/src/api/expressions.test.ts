...
const catalog: GeneratedExpressionCatalog = {
  ...
  templates: [
    ...
    {
      id: 'hello-2', type: 'prebuilt', format: 'gif', version: 'api-v1', sourceType: 'ai-original',
...
describe('mobile expression API', () => {
  it.each(['你好', '早安', '晚安', '好的', '对不起'])('已验收新词 %s 仅返回八张原 GIF，前四内置且不外搜', async (word) => {
    ...
    await writeFile(join(root, '.runtime', 'expression-assets', 'catalog.json'), JSON.stringify(production));
    const remote = { search: vi.fn(async () => []) };
    const response = await request(createApp(pool, { remoteExpressionSearch: remote }))
      .get('/api/v1/mobile/expressions/recommend').query({ q: word })
  ...
  it('返回目录并在版本未变化时返回 304', async () => {
    ...
    expect(response.body.templates.find((item: { id: string }) => item.id === 'synthesis-hot'))
  ...
  it('常用词返回 4 张含完整文字的预制原图且 GIF 优先于高热度静态图', async () => {
    ...
    expect(response.body.results[0]).toMatchObject({
      ...
      sha256: 'b'.repeat(64),
      format: 'gif',
      url: '/uploads/expression/templates/hello-2.gif',
      thumbnail_url: '/uploads/expression/thumbnails/hello-2.webp',
    });
    expect(response.body.results.length).toBeLessThanOrEqual(20);
  });

  it.each(['龘靐', '未知词'])('生僻或未知词 %s 不返回无关模板', async (query) => {
    const response = await request(createApp(pool))
  ...
  it('长句相关预制推荐保留自身文字和真实 GIF 地址', async () => {
    const remote = { search: vi.fn(async () => []) };
    const response = await request(createApp(pool, { remoteExpressionSearch: remote }))
      .get('/api/v1/mobile/expressions/recommend').query({ q: '你好呀朋友' })
      .set('X-Device-Id', USER_A);
    expect(response.status).toBe(200);
    expect(response.body.results[0]).toMatchObject({
      id: 'hello-2', type: 'prebuilt', format: 'gif', embeddedText: '你好',
      url: '/uploads/expression/templates/hello-2.gif',
    });
    expect(remote.search).not.toHaveBeenCalled();
  });

  it('未知查询不调用没有来源许可门禁的第三方搜索', async () => {
    const remote = {
