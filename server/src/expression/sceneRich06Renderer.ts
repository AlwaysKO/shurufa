...
export function sceneRich06Timeline() {
...
export function sceneRich06GridBounds(width:number,height:number) {
...
export interface SceneRich06Item {
  id: string;
  keyword: string;
  caption: string;
...
const SCENE06_ITEMS = [
  ['scene-rich-06-no', '不行', 'top-left'],
  ['scene-rich-06-slack', '摸鱼', 'top-left'],
  ['scene-rich-06-wronged', '委屈', 'bottom-left'],
  ['scene-rich-06-expect', '期待', 'bottom-left'],
] as const;
function validateSceneRich06Item(value: unknown): SceneRich06Item {
  const item = value as Partial<SceneRich06Item> | null;
  const expected = SCENE06_ITEMS.find(([id]) => id === item?.id);
  if (!item || !expected || item.keyword !== expected[1] || item.caption !== expected[1]
    ...
    || item.masterFile !== `masters/${expected[0]}.png`) throw new Error('scene-rich-06 项必须匹配固定原创文字、位置与安全路径');
  return item as SceneRich06Item;
}
export function validateSceneRich06Manifest(value: unknown) {
  const m = value as {schemaVersion?: unknown; batchId?: unknown; status?: unknown; items?: unknown[]} | null;
  if (!m || m.schemaVersion !== 1 || m.batchId !== 'scene-rich-06' || m.status !== 'review-only'
    || !Array.isArray(m.items) || m.items.length !== 4) throw new Error('仅接受 scene-rich-06 四项评审清单');
  const items = m.items.map(validateSceneRich06Item);
  if (new Set(items.map(item => item.id)).size !== 4) throw new Error('scene-rich-06 项不能重复');
  return {...m, schemaVersion: 1 as const, batchId: 'scene-rich-06' as const, status: 'review-only' as const, items};
}
...
export async function renderSceneRich06Gif(master: Buffer, value: SceneRich06Item) {
  const config=validateSceneRich06Item(value);
  const meta=await sharp(master).metadata();
  if(meta.format!=='png' || !meta.width || !meta.height) throw new Error('母版必须为至少960×720的4列3行 PNG');
  const poses=await Promise.all(sceneRich06GridBounds(meta.width,meta.height).map(bounds=>sharp(master).extract(bounds).png().toBuffer()));
  ...
  const timeline=sceneRich06Timeline();
