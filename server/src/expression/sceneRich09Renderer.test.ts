...
import { resolveSceneRich09Master } from './sceneRich09Renderer.js';
import { sceneRich09Timeline, sceneRich09GridBounds, renderSceneRich09Gif, validateSceneRich09Manifest } from './sceneRich09Renderer.js';
const item = {id:'scene-rich-09-comfort',keyword:'辛苦了',caption:'辛苦了',captionPlacement:'top-left' as const,masterFile:'masters/scene-rich-09-comfort.png',sourceType:'ai-original' as const};
const render = (b:Buffer) => renderSceneRich09Gif(b,item);
...
describe('scene rich 09 review timing', () => {
  ...
  it('cuts four by three near-square cells and rejects invalid master dimensions',()=>{
    expect(sceneRich09GridBounds(960,720)).toHaveLength(12);
    expect(sceneRich09GridBounds(960,720)[11]).toEqual({left:722,top:482,width:236,height:236});
    expect(sceneRich09GridBounds(1255,941)[11]).toEqual({left:943,top:629,width:310,height:310});
    expect(()=>sceneRich09GridBounds(960,960)).toThrow(/母版/);
    expect(()=>sceneRich09GridBounds(800,600)).toThrow(/母版/);
    const t=sceneRich09Timeline();
  ...
  it('renders independently posed GIF and actual first-frame fallback', async () => {
    ...
    expect(result.audit.metadata.delays).toEqual(sceneRich09Timeline().map(f=>f.delay));
...
const manifest=()=>({schemaVersion:1,batchId:'scene-rich-09',status:'review-only',items:[
 ['comfort','辛苦了','top-left'],['welcome','不客气','top-left'],['nervous','紧张','bottom-left'],['curious','好奇','bottom-left'],
].map(([suffix,keyword,captionPlacement])=>({...item,id:`scene-rich-09-${suffix}`,keyword,caption:keyword,captionPlacement,masterFile:`masters/scene-rich-09-${suffix}.png`,generation:{tool:'image_gen.imagegen'}}))});
it('accepts only four fixed review originals and keeps provenance',()=>{
 expect(validateSceneRich09Manifest(manifest())).toEqual(manifest());
 for(const bad of [null,{}, {...manifest(),batchId:'scene-rich-05'}, {...manifest(),status:'published'}, {...manifest(),items:[item]}, {...manifest(),items:Array(4).fill(item)}]) expect(()=>validateSceneRich09Manifest(bad)).toThrow();
 for(const change of [{caption:'错误'},{keyword:'错误'},{captionPlacement:'bottom-left'},{masterFile:'../master.png'},{sourceType:'licensed'}]){
 const m=manifest();Object.assign(m.items[0],change);expect(()=>validateSceneRich09Manifest(m)).toThrow();
 }
});

it('rejects symbolic-link masters escaping the batch root', async () => {
 const root=await mkdtemp(join(tmpdir(),'scene09-'));
 try {
  ...
  expect(await resolveSceneRich09Master(source,item.masterFile)).toBe(file);
  await rm(file); const outside=join(root,'outside.png'); await writeFile(outside,'png');
  await symlink(outside,file);
  await expect(resolveSceneRich09Master(source,item.masterFile)).rejects.toThrow(/符号链接越界/);
