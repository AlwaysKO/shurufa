...
export function sceneRich12Timeline() {
...
export function sceneRich12GridBounds(width:number,height:number) {
...
export interface SceneRich12Item {
  id: string;
  keyword: string;
  caption: string;
...
const SCENE12_ITEMS = [
  ['scene-rich-12-really-man', '真的假的', 'bottom-left'],
  ['scene-rich-12-really-line', '真的假的', 'bottom-left'],
  ['scene-rich-12-think-man', '让我想想', 'bottom-left'],
  ['scene-rich-12-think-line', '让我想想', 'bottom-left'],
] as const;
function validateSceneRich12Item(value: unknown): SceneRich12Item {
  const item = value as Partial<SceneRich12Item> | null;
  const expected = SCENE12_ITEMS.find(([id]) => id === item?.id);
  if (!item || !expected || item.keyword !== expected[1] || item.caption !== expected[1]
    ...
    || item.masterFile !== `masters/${expected[0]}.png`) throw new Error('scene-rich-12 项必须匹配固定原创文字、位置与安全路径');
  if (expected[0].endsWith('-man') && (item.personOrigin !== 'China' || item.personGender !== 'male' || item.adult !== true)) {
    throw new Error('新增人像必须记录原创中国成年男性设定');
  }
  return item as SceneRich12Item;
}
export function validateSceneRich12Manifest(value: unknown) {
  const m = value as {schemaVersion?: unknown; batchId?: unknown; status?: unknown; items?: unknown[]} | null;
  if (!m || m.schemaVersion !== 1 || m.batchId !== 'scene-rich-12' || m.status !== 'review-only'
    || !Array.isArray(m.items) || m.items.length !== 4) throw new Error('仅接受 scene-rich-12 四项评审清单');
  const items = m.items.map(validateSceneRich12Item);
  if (new Set(items.map(item => item.id)).size !== 4) throw new Error('scene-rich-12 项不能重复');
  return {...m, schemaVersion: 1 as const, batchId: 'scene-rich-12' as const, status: 'review-only' as const, items};
}
...
export async function renderSceneRich12Gif(master: Buffer, value: SceneRich12Item) {
  const config=validateSceneRich12Item(value);
  const meta=await sharp(master).metadata();
  if(meta.format!=='png' || !meta.width || !meta.height) throw new Error('母版必须为至少960×720的4列3行 PNG');
  const poses=await Promise.all(sceneRich12GridBounds(meta.width,meta.height).map(bounds=>sharp(master).extract(bounds).png().toBuffer()));
...
export async function resolveSceneRich12Master(source: string, masterFile: string) {
