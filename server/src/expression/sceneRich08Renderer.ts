...
export function sceneRich08Timeline() {
...
export function sceneRich08GridBounds(width:number,height:number) {
...
export interface SceneRich08Item {
  id: string;
  keyword: string;
  caption: string;
...
const SCENE08_ITEMS = [
  ['scene-rich-08-arrange', '安排', 'top-left'],
  ['scene-rich-08-calm', '别急', 'top-left'],
  ['scene-rich-08-hungry', '饿了', 'bottom-left'],
  ['scene-rich-08-awkward', '尴尬', 'bottom-left'],
] as const;
function validateSceneRich08Item(value: unknown): SceneRich08Item {
  const item = value as Partial<SceneRich08Item> | null;
  const expected = SCENE08_ITEMS.find(([id]) => id === item?.id);
  if (!item || !expected || item.keyword !== expected[1] || item.caption !== expected[1]
    ...
    || item.masterFile !== `masters/${expected[0]}.png`) throw new Error('scene-rich-08 项必须匹配固定原创文字、位置与安全路径');
  return item as SceneRich08Item;
}
export function validateSceneRich08Manifest(value: unknown) {
  const m = value as {schemaVersion?: unknown; batchId?: unknown; status?: unknown; items?: unknown[]} | null;
  if (!m || m.schemaVersion !== 1 || m.batchId !== 'scene-rich-08' || m.status !== 'review-only'
    || !Array.isArray(m.items) || m.items.length !== 4) throw new Error('仅接受 scene-rich-08 四项评审清单');
  const items = m.items.map(validateSceneRich08Item);
  if (new Set(items.map(item => item.id)).size !== 4) throw new Error('scene-rich-08 项不能重复');
  return {...m, schemaVersion: 1 as const, batchId: 'scene-rich-08' as const, status: 'review-only' as const, items};
}
...
export async function renderSceneRich08Gif(master: Buffer, value: SceneRich08Item) {
  const config=validateSceneRich08Item(value);
  const meta=await sharp(master).metadata();
  if(meta.format!=='png' || !meta.width || !meta.height) throw new Error('母版必须为至少960×720的4列3行 PNG');
  const poses=await Promise.all(sceneRich08GridBounds(meta.width,meta.height).map(bounds=>sharp(master).extract(bounds).png().toBuffer()));
  ...
  const timeline=sceneRich08Timeline();
