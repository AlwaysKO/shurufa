...
import { resolveSceneRich10Master } from './sceneRich10Renderer.js';
import { sceneRich10Timeline, sceneRich10GridBounds, renderSceneRich10Gif, validateSceneRich10Manifest } from './sceneRich10Renderer.js';
const item = {id:'scene-rich-10-offwork',keyword:'下班了',caption:'下班了',captionPlacement:'top-left' as const,masterFile:'masters/scene-rich-10-offwork.png',sourceType:'ai-original' as const};
const render = (b:Buffer) => renderSceneRich10Gif(b,item);
...
describe('scene rich 10 review timing', () => {
  ...
  it('cuts four by three near-square cells and rejects invalid master dimensions',()=>{
    expect(sceneRich10GridBounds(960,720)).toHaveLength(12);
    expect(sceneRich10GridBounds(960,720)[11]).toEqual({left:722,top:482,width:236,height:236});
    expect(sceneRich10GridBounds(1255,941)[11]).toEqual({left:943,top:629,width:310,height:310});
    expect(()=>sceneRich10GridBounds(960,960)).toThrow(/母版/);
    expect(()=>sceneRich10GridBounds(800,600)).toThrow(/母版/);
    const t=sceneRich10Timeline();
  ...
  it('renders independently posed GIF and actual first-frame fallback', async () => {
    ...
    expect(result.audit.metadata.delays).toEqual(sceneRich10Timeline().map(f=>f.delay));
...
const manifest=()=>({schemaVersion:1,batchId:'scene-rich-10',status:'review-only',items:[
 ['offwork','下班了','top-left'],['busy','忙着呢','top-left'],['satisfied','满足','bottom-left'],['puzzled','疑惑','bottom-left'],
].map(([suffix,keyword,captionPlacement])=>({...item,id:`scene-rich-10-${suffix}`,keyword,caption:keyword,captionPlacement,masterFile:`masters/scene-rich-10-${suffix}.png`,generation:{tool:'image_gen.imagegen'}}))});
it('accepts only four fixed review originals and keeps provenance',()=>{
 expect(validateSceneRich10Manifest(manifest())).toEqual(manifest());
 for(const bad of [null,{}, {...manifest(),batchId:'scene-rich-05'}, {...manifest(),status:'published'}, {...manifest(),items:[item]}, {...manifest(),items:Array(4).fill(item)}]) expect(()=>validateSceneRich10Manifest(bad)).toThrow();
 for(const change of [{caption:'错误'},{keyword:'错误'},{captionPlacement:'bottom-left'},{masterFile:'../master.png'},{sourceType:'licensed'}]){
 const m=manifest();Object.assign(m.items[0],change);expect(()=>validateSceneRich10Manifest(m)).toThrow();
 }
});

it('rejects symbolic-link masters escaping the batch root', async () => {
 const root=await mkdtemp(join(tmpdir(),'scene10-'));
 try {
  ...
  expect(await resolveSceneRich10Master(source,item.masterFile)).toBe(file);
  await rm(file); const outside=join(root,'outside.png'); await writeFile(outside,'png');
  await symlink(outside,file);
  await expect(resolveSceneRich10Master(source,item.masterFile)).rejects.toThrow(/符号链接越界/);
