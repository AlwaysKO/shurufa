...
import { sceneRich06Timeline, sceneRich06GridBounds, renderSceneRich06Gif, validateSceneRich06Manifest } from './sceneRich06Renderer.js';
const item = {id:'scene-rich-06-no',keyword:'不行',caption:'不行',captionPlacement:'top-left' as const,masterFile:'masters/scene-rich-06-no.png',sourceType:'ai-original' as const};
const render = (b:Buffer) => renderSceneRich06Gif(b,item);
...
describe('scene rich 06 review timing', () => {
  ...
  it('cuts four by three near-square cells and rejects invalid master dimensions',()=>{
    expect(sceneRich06GridBounds(960,720)).toHaveLength(12);
    expect(sceneRich06GridBounds(960,720)[11]).toEqual({left:722,top:482,width:236,height:236});
    expect(sceneRich06GridBounds(1255,941)[11]).toEqual({left:943,top:629,width:310,height:310});
    expect(()=>sceneRich06GridBounds(960,960)).toThrow(/母版/);
    expect(()=>sceneRich06GridBounds(800,600)).toThrow(/母版/);
    const t=sceneRich06Timeline();
  ...
  it('renders independently posed GIF and actual first-frame fallback', async () => {
    ...
    expect(result.audit.metadata.delays).toEqual(sceneRich06Timeline().map(f=>f.delay));
...
const manifest=()=>({schemaVersion:1,batchId:'scene-rich-06',status:'review-only',items:[
 ['no','不行','top-left'],['slack','摸鱼','top-left'],['wronged','委屈','bottom-left'],['expect','期待','bottom-left'],
].map(([suffix,keyword,captionPlacement])=>({...item,id:`scene-rich-06-${suffix}`,keyword,caption:keyword,captionPlacement,masterFile:`masters/scene-rich-06-${suffix}.png`,generation:{tool:'image_gen.imagegen'}}))});
it('accepts only four fixed review originals and keeps provenance',()=>{
 expect(validateSceneRich06Manifest(manifest())).toEqual(manifest());
 for(const bad of [null,{}, {...manifest(),batchId:'scene-rich-05'}, {...manifest(),status:'published'}, {...manifest(),items:[item]}, {...manifest(),items:Array(4).fill(item)}]) expect(()=>validateSceneRich06Manifest(bad)).toThrow();
 for(const change of [{caption:'错误'},{keyword:'错误'},{captionPlacement:'bottom-left'},{masterFile:'../master.png'},{sourceType:'licensed'}]){
 const m=manifest();Object.assign(m.items[0],change);expect(()=>validateSceneRich06Manifest(m)).toThrow();
 }
});
