...
import { resolveSceneRich11Master } from './sceneRich11Renderer.js';
import { sceneRich11Timeline, sceneRich11GridBounds, renderSceneRich11Gif, validateSceneRich11Manifest } from './sceneRich11Renderer.js';
const item = {id:'scene-rich-11-watch-line',keyword:'我就看看',caption:'我就看看',captionPlacement:'top-left' as const,masterFile:'masters/scene-rich-11-watch-line.png',sourceType:'ai-original' as const};
const render = (b:Buffer) => renderSceneRich11Gif(b,item);
...
describe('scene rich 11 review timing', () => {
  ...
  it('cuts four by three near-square cells and rejects invalid master dimensions',()=>{
    expect(sceneRich11GridBounds(960,720)).toHaveLength(12);
    expect(sceneRich11GridBounds(960,720)[11]).toEqual({left:722,top:482,width:236,height:236});
    expect(sceneRich11GridBounds(1255,941)[11]).toEqual({left:943,top:629,width:310,height:310});
    expect(()=>sceneRich11GridBounds(960,960)).toThrow(/母版/);
    expect(()=>sceneRich11GridBounds(800,600)).toThrow(/母版/);
    const t=sceneRich11Timeline();
  ...
  it('renders independently posed GIF and actual first-frame fallback', async () => {
    ...
    expect(result.audit.metadata.delays).toEqual(sceneRich11Timeline().map(f=>f.delay));
...
const manifest=()=>({schemaVersion:1,batchId:'scene-rich-11',status:'review-only',items:[
 ['watch-line','我就看看','top-left'],['watch-person','我就看看','top-left'],['watch-clay','我就看看','bottom-left'],['really-dog','真的假的','bottom-left'],
].map(([suffix,keyword,captionPlacement])=>({...item,id:`scene-rich-11-${suffix}`,keyword,caption:keyword,captionPlacement,masterFile:`masters/scene-rich-11-${suffix}.png`,generation:{tool:'image_gen.imagegen'}}))});
it('accepts only four fixed review originals and keeps provenance',()=>{
 expect(validateSceneRich11Manifest(manifest())).toEqual(manifest());
 for(const bad of [null,{}, {...manifest(),batchId:'scene-rich-05'}, {...manifest(),status:'published'}, {...manifest(),items:[item]}, {...manifest(),items:Array(4).fill(item)}]) expect(()=>validateSceneRich11Manifest(bad)).toThrow();
 for(const change of [{caption:'错误'},{keyword:'错误'},{captionPlacement:'bottom-left'},{masterFile:'../master.png'},{sourceType:'licensed'}]){
 const m=manifest();Object.assign(m.items[0],change);expect(()=>validateSceneRich11Manifest(m)).toThrow();
 }
});

it('rejects symbolic-link masters escaping the batch root', async () => {
 const root=await mkdtemp(join(tmpdir(),'scene11-'));
 try {
  ...
  expect(await resolveSceneRich11Master(source,item.masterFile)).toBe(file);
  await rm(file); const outside=join(root,'outside.png'); await writeFile(outside,'png');
  await symlink(outside,file);
  await expect(resolveSceneRich11Master(source,item.masterFile)).rejects.toThrow(/符号链接越界/);
