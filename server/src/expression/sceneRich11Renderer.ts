...
export function sceneRich11Timeline() {
...
export function sceneRich11GridBounds(width:number,height:number) {
...
export interface SceneRich11Item {
  id: string;
  keyword: string;
  caption: string;
...
const SCENE11_ITEMS = [
  ['scene-rich-11-watch-line', '我就看看', 'top-left'],
  ['scene-rich-11-watch-person', '我就看看', 'top-left'],
  ['scene-rich-11-watch-clay', '我就看看', 'bottom-left'],
  ['scene-rich-11-really-dog', '真的假的', 'bottom-left'],
] as const;
function validateSceneRich11Item(value: unknown): SceneRich11Item {
  const item = value as Partial<SceneRich11Item> | null;
  const expected = SCENE11_ITEMS.find(([id]) => id === item?.id);
  if (!item || !expected || item.keyword !== expected[1] || item.caption !== expected[1]
    ...
    || item.masterFile !== `masters/${expected[0]}.png`) throw new Error('scene-rich-11 项必须匹配固定原创文字、位置与安全路径');
  return item as SceneRich11Item;
}
export function validateSceneRich11Manifest(value: unknown) {
  const m = value as {schemaVersion?: unknown; batchId?: unknown; status?: unknown; items?: unknown[]} | null;
  if (!m || m.schemaVersion !== 1 || m.batchId !== 'scene-rich-11' || m.status !== 'review-only'
    || !Array.isArray(m.items) || m.items.length !== 4) throw new Error('仅接受 scene-rich-11 四项评审清单');
  const items = m.items.map(validateSceneRich11Item);
  if (new Set(items.map(item => item.id)).size !== 4) throw new Error('scene-rich-11 项不能重复');
  return {...m, schemaVersion: 1 as const, batchId: 'scene-rich-11' as const, status: 'review-only' as const, items};
}
...
export async function renderSceneRich11Gif(master: Buffer, value: SceneRich11Item) {
  const config=validateSceneRich11Item(value);
  const meta=await sharp(master).metadata();
  if(meta.format!=='png' || !meta.width || !meta.height) throw new Error('母版必须为至少960×720的4列3行 PNG');
  const poses=await Promise.all(sceneRich11GridBounds(meta.width,meta.height).map(bounds=>sharp(master).extract(bounds).png().toBuffer()));
  ...
  const timeline=sceneRich11Timeline();
...
export async function resolveSceneRich11Master(source: string, masterFile: string) {
