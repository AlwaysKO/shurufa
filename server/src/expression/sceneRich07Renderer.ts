...
export function sceneRich07Timeline() {
...
export function sceneRich07GridBounds(width:number,height:number) {
...
export interface SceneRich07Item {
  id: string;
  keyword: string;
  caption: string;
...
const SCENE07_ITEMS = [
  ['scene-rich-07-ok', '没问题', 'top-left'],
  ['scene-rich-07-tired', '心累', 'top-left'],
  ['scene-rich-07-proud', '得意', 'bottom-left'],
  ['scene-rich-07-shy', '害羞', 'bottom-left'],
] as const;
function validateSceneRich07Item(value: unknown): SceneRich07Item {
  const item = value as Partial<SceneRich07Item> | null;
  const expected = SCENE07_ITEMS.find(([id]) => id === item?.id);
  if (!item || !expected || item.keyword !== expected[1] || item.caption !== expected[1]
    ...
    || item.masterFile !== `masters/${expected[0]}.png`) throw new Error('scene-rich-07 项必须匹配固定原创文字、位置与安全路径');
  return item as SceneRich07Item;
}
export function validateSceneRich07Manifest(value: unknown) {
  const m = value as {schemaVersion?: unknown; batchId?: unknown; status?: unknown; items?: unknown[]} | null;
  if (!m || m.schemaVersion !== 1 || m.batchId !== 'scene-rich-07' || m.status !== 'review-only'
    || !Array.isArray(m.items) || m.items.length !== 4) throw new Error('仅接受 scene-rich-07 四项评审清单');
  const items = m.items.map(validateSceneRich07Item);
  if (new Set(items.map(item => item.id)).size !== 4) throw new Error('scene-rich-07 项不能重复');
  return {...m, schemaVersion: 1 as const, batchId: 'scene-rich-07' as const, status: 'review-only' as const, items};
}
...
export async function renderSceneRich07Gif(master: Buffer, value: SceneRich07Item) {
  const config=validateSceneRich07Item(value);
  const meta=await sharp(master).metadata();
  if(meta.format!=='png' || !meta.width || !meta.height) throw new Error('母版必须为至少960×720的4列3行 PNG');
  const poses=await Promise.all(sceneRich07GridBounds(meta.width,meta.height).map(bounds=>sharp(master).extract(bounds).png().toBuffer()));
  ...
  const timeline=sceneRich07Timeline();
