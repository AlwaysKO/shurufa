...
export function sceneRich09Timeline() {
...
export function sceneRich09GridBounds(width:number,height:number) {
...
export interface SceneRich09Item {
  id: string;
  keyword: string;
  caption: string;
...
const SCENE09_ITEMS = [
  ['scene-rich-09-comfort', '辛苦了', 'top-left'],
  ['scene-rich-09-welcome', '不客气', 'top-left'],
  ['scene-rich-09-nervous', '紧张', 'bottom-left'],
  ['scene-rich-09-curious', '好奇', 'bottom-left'],
] as const;
function validateSceneRich09Item(value: unknown): SceneRich09Item {
  const item = value as Partial<SceneRich09Item> | null;
  const expected = SCENE09_ITEMS.find(([id]) => id === item?.id);
  if (!item || !expected || item.keyword !== expected[1] || item.caption !== expected[1]
    ...
    || item.masterFile !== `masters/${expected[0]}.png`) throw new Error('scene-rich-09 项必须匹配固定原创文字、位置与安全路径');
  return item as SceneRich09Item;
}
export function validateSceneRich09Manifest(value: unknown) {
  const m = value as {schemaVersion?: unknown; batchId?: unknown; status?: unknown; items?: unknown[]} | null;
  if (!m || m.schemaVersion !== 1 || m.batchId !== 'scene-rich-09' || m.status !== 'review-only'
    || !Array.isArray(m.items) || m.items.length !== 4) throw new Error('仅接受 scene-rich-09 四项评审清单');
  const items = m.items.map(validateSceneRich09Item);
  if (new Set(items.map(item => item.id)).size !== 4) throw new Error('scene-rich-09 项不能重复');
  return {...m, schemaVersion: 1 as const, batchId: 'scene-rich-09' as const, status: 'review-only' as const, items};
}
...
export async function renderSceneRich09Gif(master: Buffer, value: SceneRich09Item) {
  const config=validateSceneRich09Item(value);
  const meta=await sharp(master).metadata();
  if(meta.format!=='png' || !meta.width || !meta.height) throw new Error('母版必须为至少960×720的4列3行 PNG');
  const poses=await Promise.all(sceneRich09GridBounds(meta.width,meta.height).map(bounds=>sharp(master).extract(bounds).png().toBuffer()));
  ...
  const timeline=sceneRich09Timeline();
...
export async function resolveSceneRich09Master(source: string, masterFile: string) {
