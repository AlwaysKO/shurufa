...
it('web08 script refuses arbitrary paths before reading or writing any batch',()=>{
 const result=spawnSync(process.execPath,['--import','tsx','scripts/render-web08-framed.ts','../other-batch'],{
  cwd:fileURLToPath(new URL('../../',import.meta.url)),encoding:'utf8',timeout:10000,
 });
 expect(result.status).not.toBe(0);
 expect(result.stderr).toContain('仅处理web08固定新稿，不接受参数');
});

it('rejects missing or contradictory cleanup evidence and failed sources without replacing output',async()=>{
 const server=fileURLToPath(new URL('../../',import.meta.url));
 const tmp=await mkdtemp(join(tmpdir(),'web08-gate-'));
 const batch=join(tmp,'artifacts/expression-character-trials/web-original-08-animated');
 const source=join(batch,'sources/beagle-favor/source');
 ...
 try{
  ...
  await copyFile(join(server,'scripts/render-web08-framed.ts'),join(tmp,'server/scripts/render-web08-framed.ts'));
  await writeFile(join(source,'master.png'),bytes);await writeFile(join(source,'prompt.txt'),'original generation prompt');
  await writeFile(join(batch,'framing.json'),JSON.stringify([{id:'beagle-favor',sha256:sha,scale:0.78,baselines:Array(12).fill(300)}]));
  ...
  for(const patch of [{status:'rejected'},{status:'failed'},{deletionEvidence:null},{deletionEvidence:{...valid.deletionEvidence,conversationLinkAbsent:false}},{requestedDate:''},{downloadedAt:''},{deletedAt:'2026-09-17T00:59:00Z'},{sha256:'0'.repeat(64)},{sourceType:'unknown'}]){
   ...
   const result=spawnSync(process.execPath,['--import','tsx',join(tmp,'server/scripts/render-web08-framed.ts')],{cwd:server,encoding:'utf8',timeout:10000});
   expect(result.status).not.toBe(0);
