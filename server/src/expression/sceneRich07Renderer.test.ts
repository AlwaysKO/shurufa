...
import { sceneRich07Timeline, sceneRich07GridBounds, renderSceneRich07Gif, validateSceneRich07Manifest } from './sceneRich07Renderer.js';
const item = {id:'scene-rich-07-ok',keyword:'没问题',caption:'没问题',captionPlacement:'top-left' as const,masterFile:'masters/scene-rich-07-ok.png',sourceType:'ai-original' as const};
const render = (b:Buffer) => renderSceneRich07Gif(b,item);
...
describe('scene rich 07 review timing', () => {
  ...
  it('cuts four by three near-square cells and rejects invalid master dimensions',()=>{
    expect(sceneRich07GridBounds(960,720)).toHaveLength(12);
    expect(sceneRich07GridBounds(960,720)[11]).toEqual({left:722,top:482,width:236,height:236});
    expect(sceneRich07GridBounds(1255,941)[11]).toEqual({left:943,top:629,width:310,height:310});
    expect(()=>sceneRich07GridBounds(960,960)).toThrow(/母版/);
    expect(()=>sceneRich07GridBounds(800,600)).toThrow(/母版/);
    const t=sceneRich07Timeline();
  ...
  it('renders independently posed GIF and actual first-frame fallback', async () => {
    ...
    expect(result.audit.metadata.delays).toEqual(sceneRich07Timeline().map(f=>f.delay));
...
const manifest=()=>({schemaVersion:1,batchId:'scene-rich-07',status:'review-only',items:[
 ['ok','没问题','top-left'],['tired','心累','top-left'],['proud','得意','bottom-left'],['shy','害羞','bottom-left'],
].map(([suffix,keyword,captionPlacement])=>({...item,id:`scene-rich-07-${suffix}`,keyword,caption:keyword,captionPlacement,masterFile:`masters/scene-rich-07-${suffix}.png`,generation:{tool:'image_gen.imagegen'}}))});
it('accepts only four fixed review originals and keeps provenance',()=>{
 expect(validateSceneRich07Manifest(manifest())).toEqual(manifest());
 for(const bad of [null,{}, {...manifest(),batchId:'scene-rich-05'}, {...manifest(),status:'published'}, {...manifest(),items:[item]}, {...manifest(),items:Array(4).fill(item)}]) expect(()=>validateSceneRich07Manifest(bad)).toThrow();
 for(const change of [{caption:'错误'},{keyword:'错误'},{captionPlacement:'bottom-left'},{masterFile:'../master.png'},{sourceType:'licensed'}]){
 const m=manifest();Object.assign(m.items[0],change);expect(()=>validateSceneRich07Manifest(m)).toThrow();
 }
});
