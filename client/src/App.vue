...
async function initializeUsers() {
  ...
  const isCurrent = () => authenticated.value && requestId === initializationRequestId;
  if (!isCurrent()) return;
  usersReady.value = false;
  initializationError.value = '';
  try {
    let user = currentUserId.value ? (await api.users({ id: currentUserId.value, page_size: 1 })).users[0] ?? null : null;
    if (!isCurrent()) return;
    if (!user) user = (await api.users({ page_size: 1 })).users[0] ?? null;
    if (!isCurrent()) return;
  ...
  } finally { if (isCurrent()) usersReady.value = true; }
}
watch(authenticated, (value) => {
  if (value) void initializeUsers();
  else { initializationRequestId++; usersReady.value = false; selectedUser.value = null; directoryUsers.value = []; closeDirectory(); setCurrentUserId(''); }
}, { immediate: true });
const logoutError = ref('');
async function signOut() {
  logoutError.value = '';
  try { await logout(); } catch (error) { logoutError.value = error instanceof Error ? error.message : '退出失败'; }
}
