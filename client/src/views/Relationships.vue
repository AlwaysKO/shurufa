...
const profile = ref<RelationshipAiProfile | null>(null);
const aiCandidates = ref<Array<{ text: string; source: 'zero_token' | 'ai' }>>([]);
...
function selectRelationship(row: RelationshipRow): void {
  ...
  void loadAiProfile(row.conversation_id);
}

async function loadAiProfile(conversationId: number): Promise<void> {
  try {
    const result = await api.relationshipAiProfile(conversationId);
    if (selected.value?.conversation_id === conversationId) profile.value = result.profile;
  } catch (reason) {
    if (selected.value?.conversation_id === conversationId) error.value = `AI 画像加载失败：${(reason as Error).message}`;
  }
}

async function trainAiProfile(): Promise<void> {
  if (!selected.value) return;
  const conversationId = selected.value.conversation_id;
  training.value = true; error.value = ''; saved.value = '';
  try {
    const result = await api.trainRelationshipAiProfile(conversationId);
    if (selected.value?.conversation_id !== conversationId) return;
    profile.value = result.profile;
    saved.value = `AI 画像 v${result.profile.version} 已生成`;
  } catch (reason) {
    if (selected.value?.conversation_id === conversationId) error.value = `训练失败（至少需要配置数量的双方文字消息和 DeepSeek 密钥）：${(reason as Error).message}`;
  } finally { if (selected.value?.conversation_id === conversationId) training.value = false; }
}

async function previewAiReply(): Promise<void> {
  if (!selected.value) return;
  const conversationId = selected.value.conversation_id;
  aiLoading.value = true; error.value = '';
  try {
    const result = await api.relationshipAiReplies(conversationId, contextText.value);
    if (selected.value?.conversation_id !== conversationId) return;
    aiCandidates.value = result.candidates;
    if (!result.ai_used && result.fallback_reason) saved.value = `AI 未调用，已回退历史候选：${result.fallback_reason}`;
  } catch (reason) { if (selected.value?.conversation_id === conversationId) error.value = `AI 回复预览失败：${(reason as Error).message}`; }
  finally { if (selected.value?.conversation_id === conversationId) aiLoading.value = false; }
}
  ...
  <div class="relationship-layout">
    ...
    <div class="detail-column">
      ...
      <section class="card candidate-card">
        <h3>AI 关系画像</h3>
        <p class="hint">手动训练；仅发送精简的双方文字样本，旧版本不会被覆盖。</p>
        <button class="primary" type="button" :disabled="!selected || training" @click="trainAiProfile">
          {{ training ? '训练中…' : '训练 AI 画像' }}
        </button>
        <div v-if="profile" class="ai-profile">
          <strong>v{{ profile.version }} · {{ profile.profile.summary }}</strong>
          <p>我的表达：{{ profile.profile.user_style.join('、') || '-' }}</p>
          <p>对方表达：{{ profile.profile.peer_style.join('、') || '-' }}</p>
          <p>建议语气：{{ profile.profile.preferred_tone.join('、') || '-' }}</p>
          <p>避免：{{ profile.profile.avoid.join('、') || '-' }}</p>
          <small>样本 {{ profile.source_message_count }} 条 · {{ profile.model }} · 证据 {{ profile.profile.evidence_message_ids.length }} 条</small>
        </div>
        <p v-else class="empty">尚未训练 AI 画像</p>
      </section>

      <section class="card candidate-card">
        <h3>AI 回复预览</h3>
        <p class="hint">后台预览直接模拟第二次“换一批”；AI 不可用时显示历史候选。</p>
        <div class="preview-input">
          <input v-model="contextText" maxlength="20000" placeholder="输入对方刚说的话" @keyup.enter="previewAiReply" />
          <button class="primary" type="button" :disabled="!selected || aiLoading" @click="previewAiReply">
            {{ aiLoading ? '生成中…' : '生成 AI 回复' }}
          </button>
        </div>
        <div class="candidate-list">
          <article v-for="candidate in aiCandidates" :key="`${candidate.source}:${candidate.text}`">
            <p>{{ candidate.text }}</p><span>{{ candidate.source === 'ai' ? 'AI' : '历史回退' }}</span>
          </article>
        </div>
      </section>
...
.ai-profile { display: grid; gap: 5px; margin-top: 14px; padding: 14px; border-radius: 8px; background: #f7f8ff; }
.ai-profile p { margin: 0; color: #57606f; }
.ai-profile small { color: #747d8c; }
