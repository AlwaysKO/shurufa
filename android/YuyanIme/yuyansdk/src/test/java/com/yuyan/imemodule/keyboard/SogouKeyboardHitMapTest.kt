...
class SogouKeyboardHitMapTest {
    ...
    @Test
    fun `九宫格侧栏和外围功能键为灰色角色而中央与空格为普通白键`() {
        KeyboardLoaderUtil.instance.clearKeyboardMap()
        val keyboard = KeyboardLoaderUtil.instance.getSoftKeyboard(InputModeSwitcher.MASK_SKB_LAYOUT_T9_PINYIN)
        for (code in listOf(InputModeSwitcher.USER_KEYCODE_LEFT_SYMBOL, KeyEvent.KEYCODE_DEL, KeyEvent.KEYCODE_CLEAR, KeyEvent.KEYCODE_0, InputModeSwitcher.USER_KEYCODE_NUMBER)) {
            assertEquals("key $code", com.yuyan.imemodule.entity.keyboard.KeyType.Function, keyboard.getKeyByCode(code)?.keyType)
        }
        for (code in listOf(KeyEvent.KEYCODE_A, KeyEvent.KEYCODE_G, KeyEvent.KEYCODE_SPACE)) {
            assertEquals("key $code", com.yuyan.imemodule.entity.keyboard.KeyType.Normal, keyboard.getKeyByCode(code)?.keyType)
        }
    }
