...
class ExpressionCatalogTest {
    ...
    @Test
    fun `精确预制图优先动态再按热度排序且不受旧静态热度挤占`() {
        val catalog = ExpressionCatalog(document(assets = listOf(
            asset("static-hot", type = "prebuilt", embeddedText = "测试词", heat = 999),
    ...
    fun `无预制结果时只返回限定数量的静态合成模板`() {
        val catalog = ExpressionCatalog(
            document(
                assets = listOf(
                    asset("cold", heat = 1, keywords = listOf("棉花糖")),
                    asset("hot", heat = 10, keywords = listOf("棉花糖")),
