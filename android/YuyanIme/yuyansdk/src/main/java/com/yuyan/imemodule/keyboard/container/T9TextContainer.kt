...
open class T9TextContainer(context: Context?, inputView: InputView, skbValue: Int = 0) : InputBaseContainer(context, inputView) {
    ...
    fun updateSymbolListView() {
        ...
        val viewportHeight = if (mSkbValue == InputModeSwitcher.MASK_SKB_LAYOUT_T9_PINYIN) {
            mMajorView?.getSoftKeyboard()?.let { keyboard ->
                keyboard.getKeyByCode(InputModeSwitcher.USER_KEYCODE_LEFT_SYMBOL)?.let { holder ->
                    (holder.height() - 2 * keyboard.keyYMargin).takeIf { it >= 4 }
                }
            }
        } else null
        val adapter = PrefixAdapter(context, prefixs, viewportHeight = viewportHeight)
