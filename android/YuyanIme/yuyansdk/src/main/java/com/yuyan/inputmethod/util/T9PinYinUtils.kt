...
object T9PinYinUtils {
    ...
    fun getT9Composition(composition: String, comment: String): String {
        ...
        val raw = composition.replace("'", "")
        if (raw.isNotEmpty() && raw.all { it in "ADGJMPTW" }) {
            val code = raw.map { ("ADGJMPTW".indexOf(it) + 2).digitToChar() }.joinToString("")
            T9Spelling.preedit(code, comment)?.let {
                return if (composition.endsWith("'")) "$it'" else it
            }
        }
