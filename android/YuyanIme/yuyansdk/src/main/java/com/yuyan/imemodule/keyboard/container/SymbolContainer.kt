...
class SymbolContainer(context: Context, inputView: InputView) : BaseContainer(context, inputView) {
    ...
    private fun sendSticker(sticker: StickerItem) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (result == ExpressionSendResult.WechatSubmitted) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(context, R.string.expression_wechat_submitted, Toast.LENGTH_LONG).show()
                }
            } else if (result != ExpressionSendResult.Sent) {
