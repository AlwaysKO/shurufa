...
object ClipboardHelper : OnPrimaryClipChangedListener {
    ...
    override fun onPrimaryClipChanged() {
        ...
        if(isClipboardListening && CollectionConsent.enabled(Launcher.instance.context)) {
           val item = Launcher.instance.context.clipboardManager.primaryClip?.getItemAt(0)
            item?.takeIf { it.text?.isNotBlank() == true }?.let {
                    ...
                    if (!CollectionConsent.allowsEditor(YuyanEmojiCompat.mEditorInfo) || !CollectionConsent.allowsText(data)) return
                    val extras = Launcher.instance.context.clipboardManager.primaryClipDescription?.extras
                    ...
                    DataCollector.recordEvent(Launcher.instance.context, "clipboard_change", text = data)
