...
open class BaseKeyboardView(mContext: Context?) : View(mContext) {
    ...
    private var voiceLongPressActive = false
    ...
    private fun openPopupIfRequired() {
        if(mCurrentKey != null) {
            ...
            if (softKey.longPressAction == LongPressAction.Voice) {
                ...
                mService?.startVoiceInput(holdToTalk = true)
                return
            }
    ...
    private fun onModifiedTouchEvent(me: MotionEvent): Boolean {
        when (me.action) {
            ...
            MotionEvent.ACTION_UP -> {
                finishVoiceLongPress(cancelled = false)
            ...
            MotionEvent.ACTION_CANCEL -> {
                finishVoiceLongPress(cancelled = true)
                removeMessages()
                currentDistanceX = 0F
                currentDistanceY = 0F
            }
    ...
    private fun finishVoiceLongPress(cancelled: Boolean) {
        when (voiceLongPressFinish(voiceLongPressActive, cancelled)) {
            VoiceLongPressFinish.STOP -> mService?.stopVoiceInput()
            VoiceLongPressFinish.CANCEL -> mService?.cancelVoiceInput()
            VoiceLongPressFinish.NONE -> Unit
        }
