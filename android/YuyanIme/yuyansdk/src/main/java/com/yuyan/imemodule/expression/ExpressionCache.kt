...
class ExpressionCache(cacheDir: File) {
    ...
    internal val queryRoot = File(cacheDir, "expression-query")
    ...
    fun validFile(version: String, relativePath: String, expectedSha256: String): File? =
        safeFile(version, relativePath).takeIf { it.isFile && sha256(it) == expectedSha256 }
            ?: if (ExpressionQueryCache.SHA_PATTERN.matches(expectedSha256)) {
                File(queryRoot, "originals/$expectedSha256")
                    .takeIf { it.isFile && sha256(it) == expectedSha256 }
    ...
    private fun safeFile(version: String, relativePath: String): File {
        require(version != "." && version != ".." && version.matches(Regex("[A-Za-z0-9._-]+"))) { "invalid cache version" }
