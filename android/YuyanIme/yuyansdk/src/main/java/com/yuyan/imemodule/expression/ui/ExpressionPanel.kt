...
class ExpressionPanel @JvmOverloads constructor(
    ...
    private val preparingOverlay: TextView
    ...
    private val adapter = ExpressionAssetAdapter(
        onClick = { if (!isPreparing) onAssetClick?.invoke(it) },
    ...
    init {
        ...
        preparingOverlay = findViewById(R.id.expression_preparing_overlay)
        ...
        // 文本策略只初始化一次，避免父视图 onSizeChanged 中重置子视图文本布局。
        listOf(recommendedTab, templatesTab, emojiTab).forEach { tab ->
            tab.setSingleLine(true)
            // singleLine 默认开启横向滚动；关闭后自动字号才能按实际可用宽度缩放。
    ...
    fun render(state: ExpressionPanelState, catalog: ExpressionCatalog) {
        ...
        canExpand = state.isContentVisible && recommendationVisible
        ...
        content.setPadding(0, minOf(dp(ExpressionLayoutMetrics.CONTENT_TOP_GAP_DP), content.layoutParams.height.coerceAtLeast(0)), 0, 0)
        emojiPicker.setAvailableHeight((content.layoutParams.height - content.paddingTop).coerceAtLeast(0))
        ...
        val emojiContentAccessible = expanded ||
            requireNotNull(layoutMetrics).contentHeightPx - content.paddingTop >= dp(44)
        ...
        val items = when (state.selectedTab) {
            ...
            ExpressionPanelTab.AI_SYNTHESIS -> catalog.synthesisTemplates(state.query.orEmpty())
            ExpressionPanelTab.EMOJI_SYNTHESIS -> emptyList()
        }
        findViewById<TextView>(R.id.expression_empty_results).apply {
            setTextColor(preparingOverlay.currentTextColor)
            visibility = if (!isPreparing && !showingEmoji && items.isEmpty()) View.VISIBLE else View.GONE
        }
    ...
    fun clearCallbacks() {
        onDismiss = null
        onRecommendationActionVisibilityChanged = null
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        ...
        // 同一个面板复用时，SP 对应的像素可能变化；只在配置变化时重算字号范围。
        listOf(recommendedTab, templatesTab, emojiTab).forEach { tab ->
            TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(
                tab, MIN_TAB_TEXT_SIZE_SP, MAX_TAB_TEXT_SIZE_SP, 1,
                android.util.TypedValue.COMPLEX_UNIT_SP,
            )
        }
    }
    ...
    private fun applyLayoutMetrics(measuredWidth: Int = width) {
        ...
        enableButton.minimumHeight = dp(32)
        enableButton.layoutParams = enableButton.layoutParams.apply { height = dp(32) }
        ...
        if (!isExpanded) {
            ...
            content.setPadding(0, minOf(dp(ExpressionLayoutMetrics.CONTENT_TOP_GAP_DP), metrics.contentHeightPx), 0, 0)
            emojiPicker.setAvailableHeight((metrics.contentHeightPx - content.paddingTop).coerceAtLeast(0))
        }
    ...
    private companion object {
        ...
        // 与 ExpressionLayoutMetrics.COMPACT_ACTION_DP 同步；Emoji 内容另保留 44dp。
        const val MINIMUM_TOUCH_TARGET_DP = 32
