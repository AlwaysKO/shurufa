...
internal fun encodeWebp(bitmap: Bitmap): ByteArray = ByteArrayOutputStream().use { output ->
    check(bitmap.compress(Bitmap.CompressFormat.WEBP_LOSSY, 82, output)) { "无法压缩聊天截图" }
    output.toByteArray()
}
