...
class CandidatesBarAdapter(context: Context?) :
    ...
    companion object {
        // 只调候选行汉字；1.2 = 比原字号大 20%，不影响键帽、符号或剪贴板。
        const val CANDIDATE_TEXT_SCALE = 1.1f
    }
    ...
    override fun onBindViewHolder(holder: SymbolHolder, position: Int) {
        ...
        holder.textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, instance.candidateTextSize * CANDIDATE_TEXT_SCALE)
        holder.textView.setTextColor(if((mActiveCandNo - 1).coerceAtLeast(0) == position) activeTheme.accentKeyBackgroundColor else activeTheme.keyTextColor)
