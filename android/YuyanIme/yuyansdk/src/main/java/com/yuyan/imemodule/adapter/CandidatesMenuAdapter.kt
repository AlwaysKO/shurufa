...
class CandidatesMenuAdapter(context: Context?) : RecyclerView.Adapter<CandidatesMenuAdapter.SymbolHolder>() {
    ...
    override fun onBindViewHolder(holder: SymbolHolder, position: Int) {
        ...
        val label = holder.itemView.findViewById<TextView>(R.id.candidates_menu_label)
        val showLabel = item?.skbMenuMode == SkbMenuMode.AiDoutu
        label.visibility = if (showLabel) View.VISIBLE else View.GONE
        val labelHeight = if (showLabel) adapterContext.dp(14) else 0
        icon.layoutParams = (icon.layoutParams as FrameLayout.LayoutParams).apply {
            bottomMargin = labelHeight
        }
        val availableHeight = (itemHeight - labelHeight).coerceAtLeast(1)
        val displayedIconSize = minOf(iconSize, availableHeight)
        val horizontalPadding = ((itemWidth - displayedIconSize) / 2).coerceAtLeast(0)
        val verticalPadding = ((availableHeight - displayedIconSize) / 2).coerceAtLeast(0)
