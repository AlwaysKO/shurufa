...
object KeyboardToolbarModel {
    ...
    private val emptySlotDefaults = listOf(
        SkbMenuMode.Voice,
        SkbMenuMode.PinyinHandWriting,
    )
     ...
     * 传入列表只读取，不会被原地排序或删除。
     */
    fun merge(existing: List<SkbMenuMode>): List<SkbMenuMode> {
        ...
        emptySlotDefaults.forEach { fallback ->
            if (featured.size < FEATURED_SLOT_COUNT && fallback !in featured) {
                featured += fallback
            }
        }

        return buildList(existing.size + PINNED_SLOT_COUNT) {
            add(SkbMenuMode.Emojicon)
            add(SkbMenuMode.QuickKeyboard)
            addAll(featured)
            add(SkbMenuMode.AiDoutu)
            addAll(remaining)
        }
    }
