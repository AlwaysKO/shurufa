...
class KeyRecordStack {
    ...
    /** 仅对没有锁拼音/选字/分词的原始九宫格组合提供补充候选。 */
    fun unlockedT9Digits(): String {
        if (keyRecords.isEmpty() || keyRecords.any { it !is InputKey.T9Key }) return ""
        val digits = StringBuilder()
        for (record in keyRecords) {
            val index = "ADGJMPTW".indexOf(record.toString())
            if (index < 0) return ""
            digits.append(index + 2)
        }
