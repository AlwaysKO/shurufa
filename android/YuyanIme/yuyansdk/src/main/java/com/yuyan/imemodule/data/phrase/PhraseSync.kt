...
object PhraseSync {
    ...
    fun upload(content: String) {
        DataCollector.enqueueReport("phrase_upsert", json.encodeToString(PhraseBody.serializer(), PhraseBody(content)))
    }

    /** 使用次数持久化，按文本跨环境识别。 */
    fun reportUse(content: String) {
        DataCollector.enqueueReport("phrase_use", json.encodeToString(PhraseBody.serializer(), PhraseBody(content)))
    }
