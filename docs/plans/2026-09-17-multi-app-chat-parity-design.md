...
- 用户切换第二个测试会话后，16:54:16（上海）新增抖音截图。新会话 ID 208，与首次 205 不同；从当前前景聊天容器读取标题，和新入库 display_name 精确匹配，不输出聊天名称到诊断摘要。
- 新资源实际可解码为1200×2245 WebP，文件 SHA 与数据库一致且不同于第一张。两个会话均仍保留，归属同一用户、平台均为 douyin；本次切换未覆盖旧会话或混入旧会话。
- 本次证明这两个测试会话的切换分组链路；不外推同名会话、群人数变化或所有App版本均已验收。原始取证与对比 JSON 仅保存于忽略的 `.runtime/multi-app-chat/`。

### 抖音发送触发复核：尚不能判定通过

- 用户回复“已发送”后复查会话208数据库，最新仍为16:56:58的1200×1362截图；文件SHA与记录一致。实际查看仅见旧聊天内容，没有可确认的新测试文字，因此不能把该新增截图直接当作发送触发成功证据。
- 再次只读检查设备，前台已为荣耀桌面，当前不能比对抖音发送后页面。未代用户切回或发送，需用户重新打开刚才测试会话核对。
- 本次仅能确认此前键盘展开尺寸的截图未包含键盘；新消息捕获仍待验证，不宣称发送采集验收通过。诊断JSON/XML保留在忽略的 `.runtime/multi-app-chat/`。

### 采集频率与重复内容补查（本轮继续）

- 用户强调图片去重、频率控制，不重复采集同一内容。源码确认现有300ms防抖没有硬性频率上限；同会话相同截图通过内容SHA参与消息指纹，由Room事务去重，不等于GIF变帧/键盘显隐语义去重。
- 最小修复计划：先为ViewportDebouncer增加可选最小触发间隔及单调时钟，旧调用默认不变；主聊天视口暂按推荐2000ms配置，保留最新待处理事件、末次补采和关闭取消。先写失败测试，再实现，回归适配器和Coordinator。不将模糊相似度阈值用于删除截图，避免漏掉少量新文字。
- 限频不等于动画语义去重，本批不能宣称后者已解决；持续事件防抖饥饿、延迟后前台变化也需单独核查。

### 限频实现与阶段验证

- 新增2项调度回归：间隔内多次变化只保留末次并到期执行、关闭时取消末次任务。补齐参数接口后先运行，7项中1项因旧逻辑在限制间隔内重复触发而失败；实现后与抖音适配器/Coordinator/签名联合28项通过、0失败/错误/跳过。
- 主聊天视口调度新增2000ms最小触发间隔，原300ms稳定等待保留，时钟使用单调时间；通知fallback不改。等待后重读前台窗口及树，包名/窗口/树不匹配时放弃旧任务，避免旧会话裁剪信息误套当前页面。
- 此间隔限制的是主视口调度，不是包含微信空树/通知fallback的全局截图总频率，不声称所有路线统一限频；连续变化仍可能推迟至安静后，GIF变帧和键盘显隐语义去重未解决。
- 首次并行Gradle验证出现共享增量缓存编译冲突，不能作红灯业务证据；后续串行测试完成了明确的断言失败→通过。没有修改local.properties或安装本轮新包。
- 只读数据库复核测试会话208仍为2条/2个不同图片SHA，未见相同SHA重复入库；不以无新增记录替代新消息采集成功验收。

- 独立审查发现整树一致性校验会受输入框文字变化影响：输入事件被过滤时，可能丢弃待采集末次消息。新增“等待期间输入文字允许、换会话/非聊天拒绝”回归，先复现1项失败；改为重新解析前后聊天身份、核验稳定会话key和置信度，并使用最新树裁剪，不再要求输入框文字一致。这替代上文“树不匹配即放弃”的初始实现；联合回归重新执行中。

- 修订后最终联合回归39项通过，0失败/错误/跳过，包含微信/抖音适配器、前台桥、Coordinator、签名和限频；独立复核无新增重要问题。git diff --check通过。此轮仅代码及单元回归完成，尚未构建安装APK或完成真机限频验收，GIF语义重复仍待处理。

## 用户最新修订：统一规则、首次立即采、恢复QQ

2026-09-17 用户指出全局2秒规则会漏掉短暂停留，要求三App内容/频率/去重一致，差异仅在App适配；抖音完成后恢复QQ。当前39项通过不满足新统一规则，不能据此交付。统一需求、所有入口现状、差距及后续验收集中在 `docs/plans/2026-09-17-unified-chat-capture-rules.md`，不再在本文件复制另一套参数。旧“QQ暂缓”排期及全局2秒方案被本次明确修订取代。
