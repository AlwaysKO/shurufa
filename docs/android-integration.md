...
- **地址**：查询默认 `https://my.dog8ball.com`，不再随电脑断开而失效。已启用上报独立发往线上与电脑 API，电脑地址默认 `http://127.0.0.1:3000`，通过 `adb reverse tcp:3000 tcp:3000` 访问；`localhost:5175` 是网页，不是手机 API。Debug/Release 都仅为 loopback 开放 HTTP；自定义局域网目标需 HTTPS。
...
| `service/capture/PassiveChatAccessibilityService.kt` | 页面采集入口；真机无法可靠识别消息方向时不注册页面适配器 |
