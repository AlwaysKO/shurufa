...
- **地址**：查询默认 `https://my.dog8ball.com`，不再随电脑断开而失效。网页后台“数据管理”可设置新的线上 HTTPS 根地址；手机每五分钟至多向当前线上后台读取一次配置，验证后持久化，并在同一 SQLite 事务中把尚未确认的旧线上目标迁移到新地址，电脑目标不变。旧域名必须在迁移窗口内继续可达；已彻底失效的旧域名无法自行告知新地址。线上上报始终独立执行；电脑 API 仅在端侧确认物理 USB 证据且目标 `/health` 返回 `{"status":"ok"}` 后才发送，电脑地址默认 `http://127.0.0.1:3000`，通过 `adb reverse tcp:3000 tcp:3000` 访问。常规设备使用 `USB_STATE connected/configured`，荣耀未提供该状态时以 `BATTERY_PLUGGED_USB` 兜底，但仍必须同时通过本地健康检查；无线调试且未插 USB 不触发。`localhost:5175` 是网页，不是手机 API。
...
空树兜底若检测到系统输入法窗口，截图截止到键盘实际上沿；没有键盘窗口时保留微信完整窗口底部，不再按固定 90% 高度裁切。后者会包含微信自身输入栏，但可避免最后一条长消息只保留上半段。
...
安装 APK 后至少需要手动允许通知读取；页面采集启用时才需要无障碍。部分系统在 APK 升级后会关闭已授权的无障碍服务，安装完成必须再次核对开关，应用不能静默替用户开启：
...
- 前台聊天页的进入、明确发送及内容变化会触发视口截图；不依赖通知正文，自方发送和对方回复可落在同一张可见视口截图中；
- 荣耀微信空无障碍树截图会在端侧仅识别顶部会话标题，并用规范化标题生成会话键；群名末尾成员数不参与键，避免人数变化拆组。识别失败按顶部视觉指纹隔离，不再把不同联系人和群统一写入“微信当前聊天”；
- 文字和媒体占位类型直接入库，同一通知更新幂等；
- 媒体资源先上传，资源成功后再上传引用消息；聊天报告优先于普通积压报告；
- 不请求 DeepSeek，不要求配置 `DEEPSEEK_API_KEY`。
- 后台可删除整个会话，也可删除单张截图；单图删除只解除该消息与图片的关联，共享图片仍被其他消息引用时不会删除底层资源。
...
### 2026-09-22：三键末字召回、锁音分段学习与侧栏修复

来源：用户反馈 `966` 缺“我们”、整串拼音逐字选择“泰鲮”不记忆、近期“鲮”不前移及分段后左侧只剩标点；用户要求先复核防乱词保护，并明确本次只改代码、不打包。

- **保留原保护**：锁音/分段后不再用整串数字插入本地词，避免覆盖用户指定的读音、丢失已选前缀或把后半词误学成整码。内部音节禁扩写、长词纯首字母过滤、既有可信词/原生整句规则、跨码学习至少4键、原生索引及隐私门禁均不因本次修改放开。
- **三键局部修订**：允许 `966 = wo + m` 这类前面音节完整、末音节未完成的词典召回；保留原生首项先验，将合法词典补全排在其后，避免被100个原生候选挤到末尾。不加入“我们”特判或“我哦”黑名单，不扩大三键与全码的历史互通。
- **学习与候选资格分开**：原始按键和锁音等价按键只用于最终校验；点选各段的真实读音、整词文本和原码一致，且宿主成功上屏并允许学习时，才保存整词与明确点选的段。不把一次整词选择自动拆成逐字点击；编辑/退格/取消仍作废旧分段记录。数据库结构、已有数据及可保存编码的长度范围不变。
- **原生范围内重排**：锁音/分段状态只对当前首个原生页已有项应用个人权重，不注入、删除或按文字合并异读原生项，选择仍使用原生索引；读音显示随重排首项刷新并保留已选前缀。当前“鲮”在该100项页内；不承诺把更后页的任意历史字提前到首屏。
- **左侧拼音**：兼容原生 `我们MA`、`真的MA`、`你好MA`、`泰JGMG` 这类汉字与剩余按键相邻的状态，不再要求整段全大写才提取待选拼音。
- 验证命令、结果与限制见 `docs/plans/2026-09-22-t9-segment-learning.md`。本次未构建新APK、未安装，不代表手机当前版本已更新；之前未成功保存的历史词不能凭此次修复自动找回。

### 2026-09-22：个人词库添加、指定手机增量并集

- 后台手动添加词语/逐字拼音，选择手机增量下发；本地和线上均可纯追加，不自动绑定组、不代替主控停用/删除。页面区分投递排队、手机确认和旧端待升级。
- Android数据库v8独立保存并集，不伪造/复制点击次数；旧换机恢复词在快照替换前保留，源缺项不删已有词。明确停用/删除仅由显式启用恢复，不能靠空策略快照复活。
- 两个目标各自协商、下载、事务落词、确认并保存游标；失败重试幂等。手工词在本次九宫格拼写边界内纳入候选并提高基础优先级，不放开内部音节扩写，不改锁音/分段注入规则。
- 实现、审查发现、命令与验收边界见 `docs/plans/2026-09-22-dictionary-additive-sync.md`。本次遵循用户不打包要求，未部署/迁移真实库/安装手机，真实手机候选效果需升级后另验，不因后台排队成功宣称已生效。
